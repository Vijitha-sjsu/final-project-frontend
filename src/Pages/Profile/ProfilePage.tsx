...
function ProfileHeader() {
  ...
  const handleSave = (formData) => {
    const url = `https://follow-service:443/api/users/${userData.userId}`;
...
export default function ProfilePage({ profileData }) {
  ...
  const fetchUserPosts = async () => {
    ...
    setError('');
    try {
      const response = await axios.get(`https://post-service:443/api/post/getUserPosts/${userData.userId}`);
  ...
  const fetchFollowers = async () => {
    ...
    try {
      const response = await axios.get(`https://follow-service:443/api/users/${userData.userId}/followers`);
  ...
  const fetchFollowing = async () => {
    if (!userData.userId) return;
    try {
      const response = await axios.get(`https://follow-service:443/api/users/${userData.userId}/following`);
  ...
  const handleDeletePostClick = async (postId) => {
    try {
        await axios.delete(`https://post-service:443/api/post/deletePost/${userData.userId}/${postId}`);
        fetchUserPosts();
    } catch (error) {
