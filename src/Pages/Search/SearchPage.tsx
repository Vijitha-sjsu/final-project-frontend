...
const SearchPage = () => {
  ...
  const handleFollowUser = async (userIdToFollow) => {
    setIsLoading(true);
    try {
      await axios.post(`https://follow-service/api/users/${userData.userId}/follow/${userIdToFollow}`);
  ...
  const handleSearchSubmit = async () => {
    ...
    setErrorMessage('');
    try {
        const searchRequest = axios.get(`https://follow-service/api/users/search?keyword=${searchQuery}`);
        const userRequest = axios.get(`https://follow-service/api/users/getUser/${userData.userId}`);
